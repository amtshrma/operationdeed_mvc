	...
	if(count($loanDetailFull)){
		foreach($loanDetailFull as $i => $loan) {?>
			<tr>
				...
				<td class="small-cell v-align-middle">
					<?php
						echo $this->Html->link(ucfirst($loan['ShortApplication']['applicant_first_name']) . ' '.ucfirst($loan['ShortApplication']['applicant_last_name']),array('controller'=>'homes','action'=>'viewUser',base64_encode(base64_encode($loan['Loan']['borrower_id']))),array('target'=>'blank')). '<br/>'.$loan['ShortApplication']['applicant_email_ID']?></p>	
				</td>
				<td class="small-cell v-align-middle">
