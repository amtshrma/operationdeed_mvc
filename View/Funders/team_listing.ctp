    ...
    <div class="col-md-12">
        <div class="row">
            <div class="col-md-12">
				...
				<?php
						//echo $this->Html->link('Create a team', array('controller'=>'funders', 'action'=>'team_assignment'), array('class'=>'btn')); ?>
						</div>		       
        ...
        <div id="accordion">
			...
			<div class="col-sm-12">
				...
				<div class="col-sm-1">
				<?php //echo $this->Html->link('<i class="fa fa-fw fa-edit"></i>', array('controller' =>'funders' , 'action' => 'team_assignment', base64_encode(base64_encode($team['Team']['id']))), array("title"=>"Edit", 'alt'=>'Edit', 'class' =>'edit', 'escape' => false)); ?></div>
			</div>			
			<div class="col-sm-12">
				<?php
				$displayArr = array();
				...
				if(!empty($displayArr)) {
					...
					foreach($displayArr as $memberType=>$memberArr) {
						...
						foreach($memberArr as $memberId) {
							...
							$displayUser = $arrUser['User']['first_name'].' '.$arrUser['User']['last_name'].' - '.$arrUser['User']['email_address'];
							
							echo '<p><i class="fa fa-male"></i> '.$displayUser.'</p>';
						}
					}
				} ?>
			</div>
